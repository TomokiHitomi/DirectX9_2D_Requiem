...
HRESULT InitScore(int type)
{
	...
	g_posScore = D3DXVECTOR3((float)SCORE_POS_X + (float)SCOREBOARD_POS_X, (float)SCORE_POS_Y + (float)SCOREBOARD_POS_Y, 0.0f);
	g_nScore = 0;
	g_bPosMoveFlag = true;
	g_bScoreStop = false;
...
void UpdateScore(void)
{
	...
	if (g_bPosMoveFlag && !result->bPosMoveFlag)
	{
		g_posScore += result->vecPosMove;
		MakeVertexScore();
		g_bPosMoveFlag = false;
		SetTotal(1, g_nScore);
	}
...
void DrawScore(void)
{
	...
	for (int i = 0; i < SCORE_DIGIT; i++)
	{
		...
		// ポリゴンの描画
		pDevice->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, NUM_SCORE, g_vertexWkScore[i], sizeof(VERTEX_2D));
	}
...
HRESULT MakeVertexScore(void)
{
	float fWidth = TEXTURE_SCORE00_SIZE_X;	// 数字の横幅

	// 桁数分処理する
	for (int i = 0; i < SCORE_DIGIT; i++)
	{
		// 頂点座標の設定
		g_vertexWkScore[i][0].vtx.x = -fWidth * i + g_posScore.x;
		g_vertexWkScore[i][0].vtx.y = g_posScore.y;
		g_vertexWkScore[i][0].vtx.z = 0.0f;
		g_vertexWkScore[i][1].vtx.x = -fWidth * i + g_posScore.x + TEXTURE_SCORE00_SIZE_X;
		g_vertexWkScore[i][1].vtx.y = g_posScore.y;
		g_vertexWkScore[i][1].vtx.z = 0.0f;
		g_vertexWkScore[i][2].vtx.x = -fWidth * i + g_posScore.x;
		g_vertexWkScore[i][2].vtx.y = g_posScore.y + TEXTURE_SCORE00_SIZE_Y;
		g_vertexWkScore[i][2].vtx.z = 0.0f;
		g_vertexWkScore[i][3].vtx.x = -fWidth * i + g_posScore.x + TEXTURE_SCORE00_SIZE_X;
